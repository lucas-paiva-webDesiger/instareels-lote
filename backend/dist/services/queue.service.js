...
class QueueService {
    ...
    async recalculateQueue(accountId) {
        const account = await prisma.instagramAccount.findUnique({ where: { id: accountId } });
        if (!account)
            return;
        const settings = account.settings; // { intervalMinutes: 30, startTime: "08:00", endTime: "22:00", days: [1,2,3,4,5] }
        ...
        let currentTime = (0, dayjs_1.default)().tz(DEFAULT_TZ);
        if (lastPublished && lastPublished.publishedAt) {
            const lastTime = (0, dayjs_1.default)(lastPublished.publishedAt).tz(DEFAULT_TZ);
    ...
    getNextValidTime(baseTime, settings) {
